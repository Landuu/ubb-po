...
def second2():
    ...
    if image is None:
        ...
        # Laplacian Operator
        laplacian = cv2.Laplacian(image, cv2.CV_64F)
        laplacian_8bit = cv2.convertScaleAbs(laplacian)  # Convert Laplacian to 8-bit format
        ...
        psnr_laplacian = metrics.peak_signal_noise_ratio(image, laplacian_8bit)
        ssim_laplacian = metrics.structural_similarity(image, laplacian_8bit, data_range=255)  # Specify data range for SSIM
        ...
        plt.subplot(2, 4, 4), plt.imshow(laplacian_8bit, cmap='gray')
        plt.title(f'Laplace\'a\nPSNR: {psnr_laplacian:.2f}\nSSIM: {ssim_laplacian:.2f}'), plt.xticks([]), plt.yticks([])
